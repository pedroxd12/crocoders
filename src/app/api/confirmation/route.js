...
// Envía el correo de confirmación de inscripción a un evento.
//
...
// HMAC-SHA256 (PAYLOAD_SECRET) por /api/eventos/register. De ahí se obtiene el
// id de inscripción y TODOS los datos del correo —destinatario incluido— se
// leen de la base de datos. El cuerpo del mensaje no contiene nada que el
// cliente pueda controlar.
...
export async function POST(request) {
  ...
  const payload = verificarQrToken(body?.qrToken, secret);
  if (!payload) return respuestaGenerica();

  const qrToken = body.qrToken;
  const { id: inscripcionId, eid: eventoId, ts } = payload;
  ...
  const rlInscripcion = rateLimit(request, {
    key: `confirmation:inscripcion:${inscripcionId}`,
    limit: 3,
    windowMs: 60 * 60 * 1000,
  });
  ...
  try {
    ...
    // El QR se genera a partir del token ya verificado.
    const attachments = [];
    try {
